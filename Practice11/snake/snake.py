...
def draw_food(food):
    ...
    rect   = cell_rect(c, r)
    centre = rect.center
    pygame.draw.circle(screen, food.type["color"], centre, CELL//2-2)
    pygame.draw.circle(screen, (255, 255, 255), (centre[0]-3, centre[1]-3), 3)
...
class Food:
    def __init__(self, snake_body):
        self.types=[
            {"color": RED, "score":10, "weight":60, "lifetime":5000},
            {"color":GOLD, "score":25, "weight":30, "lifetime":4000},
    ...
    def spawn(self, snake_body):
        ...
        self.pos = spawn_food(snake_body)
        self.spawn_time= pygame.time.get_ticks()

    def is_expired(self):
        return pygame.time.get_ticks()-self.spawn_time > self.type["lifetime"]
    ...
    while running:
        ...
        if now - last_move_time >= move_delay():
            ...
            if new_head == food.pos:
                score       += food.type["score"]      # More points at higher levels
                foods_eaten += 1
                food.spawn(body)           # Respawn food on empty cell
            ...
            if food.is_expired():
                food.spawn(body)          # Remove tail (snake doesn't grow)
